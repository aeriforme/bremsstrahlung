...
def one_BS_num_limit(E1 = 100*1e3*eV, Z = 29, Z_star = 0, n_at = 8.47e+28, T = 0, N = 50):
    ...
    def k_dsigma_dk(t, t0, f, g0, k): 
        ...
        q2 = p**2 + p0**2 + k**2 - 2*p0*k*t0 + 2*p*k*t - 2*p*p0*(1 + t*t0*cos(f))
    

        term1 = p**2  * t**2  / (g  - p  )**2   * (4*g0**2 - q2) 
        term2 = p0**2 * t0**2 / (g0 - p0 )**2 * (4*g**2  - q2)
        term3 = -2*p*p0*t*t0*cos(f)*(4*g*g0-q2+2*k**2) / (g - p) / (g0 - p0)
        term4 = 2*k**2*(p**2*t**2 + p0**2*t0**2) / (g - p) / (g0 - p0)
        ...
        one_minus_F = FT_V * q2 * 4*pi*epsilon_0*r_c/(e*Z) 
        return Z**2 * alpha * r_e**2 /(2*pi) * one_minus_F**2 * curly * p / p0 / q2**2 * t * t0  


    f  = np.linspace(0, 2*pi, N)
    ...
    for k in k_vec:
        g = g0 -k     
        t  = np.linspace(0, 1./g,   N)
        t0 = np.linspace(0, 1./g,   N)
